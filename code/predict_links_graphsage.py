...
def get_node_features(G):
    """Create low-dimensional node features using node degree."""
...
def load_model(model_path="models/graphsage_link_predictor.pt"):
    """Load the trained GraphSage model from checkpoint."""
...
def process_graph(gml_file_path):
    ...
    # Use node degree features instead of identity matrix
    node_features = get_node_features(G).to(device)
...
def main(gml_file_path, model_path="models/graphsage_link_predictor.pt", top_k=None, optimal_threshold=0.5029):
    ...
    output_file = os.path.splitext(gml_file_path)[0] + "_with_predictions_graphsage.gml"
    nx.write_gml(enhanced_graph, output_file)
    print(f"\nEnhanced graph saved to {output_file}")
    
    # Also save predictions to text file
    text_output_file = os.path.splitext(gml_file_path)[0] + "_predictions_graphsage.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/graphsage_link_predictor.pt", help="Path to the trained model file")
