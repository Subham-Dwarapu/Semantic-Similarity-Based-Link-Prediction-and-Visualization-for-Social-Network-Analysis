...
class GATLayer(nn.Module):
    def __init__(self, in_features, out_features, dropout=0.6, alpha=0.2, concat=True):
        ...
        # Split attention parameters for source and target
...
def load_model(model_path="models/gat_link_predictor.pt"):
...
def main(gml_file_path, model_path="models/gat_link_predictor.pt", top_k=None, optimal_threshold=0.5029):
    ...
    output_file = os.path.splitext(gml_file_path)[0] + "_with_predictions_gat.gml"
    nx.write_gml(enhanced_graph, output_file)
    print(f"\nEnhanced graph saved to {output_file}")
    
    # Also save predictions to text file
    text_output_file = os.path.splitext(gml_file_path)[0] + "_predictions_gat.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="models/gat_link_predictor.pt", help="Path to the trained model file")
